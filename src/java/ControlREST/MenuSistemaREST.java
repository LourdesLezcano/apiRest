...
@Path("menuSistema")
public class MenuSistemaREST {

    private MenuSistemaDTO dto;
    private MenuSistemaDAOIMP dao;
...
//    }
    ...
    public String insertar(InputStream json, @QueryParam("token") String token) {
        ...
        tokenDAO = new TokenDAOIMP();
        Gson gson = new Gson();
        dto = gson.fromJson(Util.getJson(json), MenuSistemaDTO.class);
        if (tokenDAO.verificarToken(token) == true) {
            dao = new MenuSistemaDAOIMP();
    ...
    public String eliminar(@QueryParam("id") Integer id, @QueryParam("token") String token) {
        ...
        if (tokenDAO.verificarToken(token) == true) {
            dao = new MenuSistemaDAOIMP();
            Boolean resp = dao.eliminarRegistro(new MenuSistemaDTO(id));
            if (resp == false) {
